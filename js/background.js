(function(){
  var canvas=document.getElementById('darkveil-canvas');
  if(!canvas)return;
...
`;
  ...
  var uScanFreq=gl.getUniformLocation(prog,'uScanFreq');
  var uWarp=gl.getUniformLocation(prog,'uWarp');

  var hueShift=-5,noiseIntensity=0,scanlineIntensity=0,speed=0.5,scanlineFrequency=0,warpAmount=0,resolutionScale=1;

  function resize(){
    var w=Math.floor(window.innerWidth*resolutionScale);
    var h=Math.floor(window.innerHeight*resolutionScale);
